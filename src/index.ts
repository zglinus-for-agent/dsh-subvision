...
// Pull in the @deepseek-ai/dsh-subagent declarations that augment cordis Context
// with the `subagents` service (declared via `declare module '@deepseek-ai/cordis'`).
...
function parseModelOverride(value: string | undefined, parentProvider: string | undefined): { provider?: string; model?: string } {
...
function titleFor(imagePath: string, marker: string): string {
...
export function apply(ctx: Context, config: PluginConfig): void {
  ...
  const registries = new Map<string, VisionRegistry>();
  const registryFor = (parentSessionId: string): VisionRegistry => {
  ...
  installDevicesApi(ctx as Parameters<typeof installDevicesApi>[0], { currentConfig });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (parent === undefined) throw new Error("image_recognize requires a calling agent (exec.agent was undefined)");
      ...
      const run = withLock(lockKey, async (): Promise<{ action: "created" | "followup"; childId: string }> => {
