...
export declare const CATALOG_TTL_MS = 60000;
/** Whole provider list timeout (remote catalogs such as opencode are slow). */
export declare const MODELS_TIMEOUT_MS = 10000;
/** Per-model capability probe timeout. */
export declare const MODEL_INFO_TIMEOUT_MS = 4000;
/** `ctx.llm` is optional; never let a missing service throw. */
export declare function llmOf(ctx: Context): LlmLike | undefined;
export declare function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T | null>;
/** Load one provider's model ids plus the vision-capable subset (cached). */
export declare function loadProviderCatalog(llm: LlmLike, providerId: string, cache: Map<string, ProviderCatalogEntry>, ttlMs?: number): Promise<ProviderCatalogEntry>;
/**
 ...
 */
export declare function firstVisionModel(ctx: Context, cache: Map<string, ProviderCatalogEntry>, ttlMs?: number): Promise<{
    provider: string;
    model: string;
} | undefined>;
